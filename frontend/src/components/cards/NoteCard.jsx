...
const NoteCard = ({ note, onApprove, onReject, showActions = false }) => {
    ...
    // In student browsing mode, clicking the card should open the note detail page,
    // even if `fileUrl` is missing for some reason.
    const canOpenDetail = !showActions && _id;
    const viewHref = canOpenDetail ? `/student/notes/${_id}` : null;

    return (
        <div
            className="note-card card-hover"
            style={{ cursor: canOpenDetail ? 'pointer' : 'default', position: 'relative' }}
        >
            {canOpenDetail && (
                <Link
                    to={viewHref}
                    aria-label={`View note: ${title}`}
                    style={{ position: 'absolute', inset: 0, zIndex: 5, textDecoration: 'none', display: 'block' }}
                />
            )}
