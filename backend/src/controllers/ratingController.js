...
/** Aggregation $match must use ObjectId — plain strings often match zero docs. */
const toObjectId = (id) => {
...
const buildNoteSummary = async (noteId, userId) => {
    ...
    if (!nid || !uid) {
        throw new Error('Invalid note or user id for rating summary');
    }
    ...
    const [summaryRows, currentUserRow] = await Promise.all([
        ...
        Rating.findOne({ noteId: nid, userId: uid }).select('_id rating').lean()
    ]);
...
export const createRating = async (req, res) => {
    try {
        ...
        const noteOid = toObjectId(incomingNoteId);
        ...
        const note = await Note.findById(noteOid).select('_id title uploadedBy status').lean();
        if (!note) {
        ...
        const existingRating = await Rating.findOne({ noteId: noteOid, userId });
